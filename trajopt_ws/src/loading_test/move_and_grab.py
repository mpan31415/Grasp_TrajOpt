...
CLOSED = 0
OPEN = 0.1
MAX_EFFORT = 100 # percentage of max torque of the motor

got_shelf = False
got_box = False


################################### RETURNS YAW FROM GIVEN QUATERNION ############################
def get_yaw(x, y, z, w):
...
################################### POINT HEAD AT GIVEN HEAD JOINT POSITIONS ################################
def point_head(head_client, head_joint_positions):
    ...
    rospy.loginfo("waiting for point_head action result...")
...
def move_hand(type):
    ...
    rospy.loginfo("waiting for gripper action result...")


############################### SCAN THE ENVIRONMENT (STATIONARY HEAD) ################################3
def scan():
    ...
    #############################################################################################
    ############################## CAN WE GET THE OBJECT POSITION? ##############################
    try:
...
################################ SWEEPS AROUND THE HEAD AND CALLS SCAN FUNCTION AT EACH POSITION ###############################
def sweep_and_scan():
...
def move_box_to_basket(object_name, box_x, box_y, box_z):
    ...
    ################################# SECOND PHASE OF TRAJECTORY ###########################
    move_group_2 = moveit_commander.MoveGroupCommander("arm")
    
    move_group_2.set_max_velocity_scaling_factor(0.1)
    move_group_2.set_planning_time(20)
    #move_group.set_planner_id(planner_id)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if (got_box_1 and got_box_2 and got_shelf):
            ...
            move_box_to_basket(object_name_2, box_2_pos[0]- 0.03, box_2_pos[1], box_2_pos[2])
            move_box_to_basket(object_name, box_1_pos[0], box_1_pos[1], box_1_pos[2])
